...
struct row* row_create(struct table* table) {
    struct row* new_1 = (struct row*)malloc(sizeof(struct row));
    struct row_header* row_header =(struct row_header*) malloc(sizeof(struct row_header));
    row_header->is_available = true;
    new_1->table = table;
    new_1->data = (void** ) malloc(table->schema->length);
    new_1->row_header = row_header;
    return new_1;
}
...
char* row_select(struct query *query, bool show_output, char* buf) {
    ...
    if (is_column_present) {
        ...
        const void* val = query->value[0];
        buf = select_execute(query->table->table_header->database->source_file, query->table, offset, size,
                       val, data_type, query->number, show_output, buf);
...
char * row_update(struct query *query, bool show_output, char *buf){
    ...
    if (is_column_present_one && is_column_present_two) {
        ...
        buf = update_execute(query->table->table_header->database->source_file, query->table, query_one,
...
char* row_delete(struct query *query, bool show_output, char* buf) {
    ...
    if (is_column_present) {
        struct query_params* query_params = (struct query_params*)malloc(sizeof(struct query_params));
        ...
        buf = delete_execute(query->table->table_header->database->source_file, query->table, query_params, query->value[0], buf);
