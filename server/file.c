...
enum file_status file_update_last_page(FILE *file, uint32_t old_number, uint32_t new_1) {
    struct page_header* page_header = (struct page_header*)malloc(sizeof(struct page_header));

    fseek(file, (old_number - 1) * DEFAULT_PAGE_SIZE_BYTES, SEEK_SET);
    fread(page_header, sizeof(struct page_header), 1, file);
    page_header->next_page_number = new_1;
...
enum file_status page_header_write_real(FILE *file, struct database_header* database_header, struct table_header* table_header) {
    ...
    struct page_header* new_1 = NULL;

    if (database_header->last_page_number == 1) {
        fseek(file, sizeof(struct database_header), SEEK_SET);
    } else fseek(file, (database_header->last_page_number - 1) * DEFAULT_PAGE_SIZE_BYTES, SEEK_SET);

    struct page_header* real_page =(page_header*) malloc(sizeof(struct page_header));

    fread(real_page, sizeof(struct page_header), 1, file);
    if (!is_enough_space(real_page, sizeof(struct table_header))) {
        new_1 = page_add_real(database_header);
        real_page = new_1;
    ...
    for (size_t i = 0; i < left->schema->count; i++) {
        if (offset_one != left_offset) {
            switch (left->schema->start[i].data_type) {
                case INTEGER:
                    integer_output(begin_left, offset_one, buf);
                    break;
                case BOOLEAN:
                    boolean_output(begin_left, offset_one, buf);
                    break;
                case DOUBLE:
                    double_output(begin_left, offset_one, buf);
                    break;
                case VARCHAR:
                    varchar_output(begin_left, offset_one, buf);
                    break;
            }
        } else {
            for (size_t j = 0; j < right->schema->count; j++) {
                if (offset_two != right_offset) {
                    switch (right->schema->start[j].data_type) {
                        case INTEGER:
                            integer_output(begin_right, offset_two, buf);
                            break;
                        case BOOLEAN:
                            boolean_output(begin_right, offset_two, buf);
                            break;
                        case DOUBLE:
                            double_output(begin_right, offset_two, buf);
                            break;
                        case VARCHAR:
                            varchar_output(begin_right, offset_two, buf);
                            break;
                    }
                }
...
char * integer_query_join_compare(char *left_row, char *right_row, struct query_params *left_query,
                                  struct query_params *right_query, struct table *left, struct table *right,
                                  char *buf) {
...
char* query_join_attempt(FILE *file, struct table* left, struct table* right, struct query_params* left_query, struct query_params* right_query, char* left_row, char* buf) {
    struct row_header* row_header = (struct row_header*) malloc(sizeof(struct row_header));

    uint32_t ptr = sizeof(struct page_header) + sizeof(uint16_t) + sizeof(struct column) * right->schema->count;

    char* row_ptr = (char*) malloc(right->schema->length);
...
char*query_join(FILE *file, struct table* left, struct table* right, struct query_params* left_query, struct query_params* right_query, char* buf) {
    ...
    struct row_header* row_header = (struct row_header*) malloc(sizeof(struct row_header));
    char* row_ptr =(char*) malloc(left->schema->length);
...
enum file_status table_page_write(FILE *file, struct page_header* page_header, struct schema* schema) {
    ...
    struct column* columns = (column*)malloc(size);
...
enum file_status table_read_columns(FILE *file, struct table* table) {
    struct column* columns = (column*)malloc(sizeof(struct column) * table->table_header->schema.count);
...
enum file_status row_write_to_page(FILE *file, uint32_t page_number, struct row* row) {
    ...
    if (fread(page_header, sizeof(struct page_header), 1, file) == 1) {
        ...
        if (!is_enough_space(page_header, size)) {
            ...
            new_1 = page_add(row->table->table_header, row->table->table_header->database->database_header);
            ...
            struct column* columns =(column*) malloc(columns_size);
...
void update_query(char *begin,const void *value, struct query_params *query, struct table *table, uint32_t ptr,
    ...
    if (show_output)
        data_output(begin, table->schema->start, table->schema->count, buf);
}
char * select_execute(FILE *file, struct table *relation, uint32_t offset, uint16_t column_size,const void *value,
    ...
    char* row_ptr = (char*)malloc(relation->schema->length);
    struct row_header* row_header = (struct row_header*)malloc(sizeof(struct row_header));

    struct page_header* page_header =(struct page_header*) malloc(sizeof(struct page_header));

    fseek(file, (relation->table_header->page_number_first - 1) * DEFAULT_PAGE_SIZE_BYTES, SEEK_SET);

    fread(page_header, sizeof(struct page_header), 1, file);

    fseek(file, (relation->table_header->page_number_first - 1) * DEFAULT_PAGE_SIZE_BYTES + sizeof(struct page_header) + sizeof(uint16_t) + sizeof(struct column) * relation->schema->count, SEEK_SET); //передвинулись на начало строк
...
char* update_execute(FILE *file, struct table *table, struct query_params *first, struct query_params *second,
    ...
    char* row_ptr = (char*)malloc(table->schema->length);
            ...
            switch (first->data_type) {
                case INTEGER:
                    if (integer_compare(row_ptr, values[0], first->offset)) {
                        update_query(row_ptr, values[1], second, table, current_ptr, page_header->page_number, show_output, buf);
                        result_count++;
                    }
                    break;
                case BOOLEAN:
