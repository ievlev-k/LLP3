...
terminal:
|   TYPE { $$ = new_type((enum type)($1)); }
|   INT { $$ = new_number($1); }
|   FLOAT typetype{ $$ = new_float_number($1); }
|   BOOL { $$ = new_bool($1); }
...
filter_condition:
|   filter_condition "&&" filter_condition { $$ = new_where((enum logic_op_type)($2), $1, $3); }
|   filter_condition "||" filter_condition { $$ = new_where((enum logic_op_type)($2), $1, $3); }
|   "(" filter_condition ")" { $$ = $2; }
|   logic_statement
;
...
logic_statement:
|   column CMP terminal { $$ = new_compare((enum compare_op_type)($2), $1, $3); }
|   terminal CMP column { $$ = new_compare(switch_cmp_mode((enum compare_op_type)($2)), $1, $3); }
|   column CMP column { $$ = new_compare((enum compare_op_type)($2), $1, $3); }
;
